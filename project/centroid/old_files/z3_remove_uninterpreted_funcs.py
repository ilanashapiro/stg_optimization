...
def rank_vars_for_level_submatrix(idx_node_submap):
	max_bit_size = len(idx_node_submap).bit_length()
	return [z3.BitVec(f"rank_{i}", max_bit_size) for i in idx_node_submap.valkeysues()]

def instance_parent_vars_level_submatrix(idx_node_submap1, idx_node_submap2, parent_num):
	max_bit_size = len(idx_node_submap1).bit_length() # parent levele mapping
	return [z3.BitVec(f"instance_parent{parent_num}_{child_index}", max_bit_size) for child_index in idx_node_submap2.values()]
...
def add_instance_parent_count_constraints(combined_submatrix, 
																					idx_node_submap1, 
																					idx_node_submap2, 
																					instance_parent1_vars,
																					instance_parent2_vars,
	...
	for i_subA2, node_id2 in idx_node_submap2.items():
		...
		for list_idx, (parent_index1_subA1, parent_id1) in enumerate(list(idx_node_submap1.items())):
			...
			opt.add(z3.Implies(z3.And(parent_count == 1, parent_condition1), z3.And(instance_parent1_vars[i_subA2] == parent_index1_subA1, instance_parent2_vars[i_subA2] == parent_index1_subA1))) 
			...
			for parent_index2_subA1, parent_id2 in list(idx_node_submap1.items())[list_idx+1:]: # ensure we're only looking at distinct tuples of parents, otherwise we are UNSAT
				...
				opt.add(z3.Implies(z3.And(parent_condition1, parent_condition2, parent_count == 2), 
													z3.And(instance_parent1_vars[i_subA2] == parent_index1_subA1, instance_parent2_vars[i_subA2] == parent_index2_subA1)))

def add_intra_level_linear_chain2(A_submatrix, idx_node_submap, ranks):
	...
	for i_subA in idx_node_submap.keys():
			for j_subA in idx_node_submap.keys():
					if i_subA != j_subA:
							edge_i_to_j = A_submatrix[i_subA, j_subA]
							# Ensure the rank of i is less than the rank of j if there's an edge from i to j
							opt.add(z3.Implies(edge_i_to_j, ranks[i_subA] < ranks[j_subA]))

	# Enforce distinct ranks for all nodes in the level
	# opt.add(z3.Distinct(ranks))

	# Optionally, you can also ensure that ranks start from 0 and are continuous up to num_partition_nodes - 1
	# for rank in ranks:
	# 		opt.add(rank >= 0)
	# 		opt.add(rank < num_partition_nodes)
...
def add_intra_level_linear_chain(level, 
	...
	for i_subA in idx_node_submap.keys():
		...
		is_intermediate_chain_node = z3.And(z3.Not(start_nodes[i_subA]), z3.Not(end_nodes[i_subA]))
		opt.add(is_intermediate_chain_node == z3.And(((num_incoming_edges == 1), (num_outgoing_edges == 1))))
...
for (parent_level, child_level), (A_combined_submatrix, combined_idx_node_submap) in sorted(A_adjacent_instance_submatrices_list.items()):
	...
	if parent_level not in level_states:
		add_intra_level_linear_chain2(parent_level, A_submatrix1, idx_node_submap1)
	if child_level not in level_states:
		add_intra_level_linear_chain2(child_level, A_submatrix2, idx_node_submap2)
