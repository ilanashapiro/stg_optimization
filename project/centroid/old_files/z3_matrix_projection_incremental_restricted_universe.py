...
opt = z3.Optimize()
opt.set('timeout', 20000) # in milliseconds. 300000ms = 5mins
...
# centroid matrix A, so that we can overcome the performace cost of having unbounded complexity of infinite domain/range functions
# domains is the list of domain universes for each input param to the function
# def constrain_function_to_universe(f, domain_universe, range_universe):
# 	# constrain domain of f to domain_universe
# 	for input_value in domain_universe:
# 		opt.add(z3.Or([z3.Int(i) == input_value for i in domain_universe]))
	
# 	# constrain range of f to range_universe
# 	for input_value in domain_universe:
# 		# Use a variable to represent the function's output
# 		output = f(z3.Int(input_value))
# 		opt.add(z3.Implies(z3.And(x == z3.Int(i)), z3.And(
# 				output >= 0,
# 				output < range
# 		)))
...
def add_prototype_to_instance_constraints(level, instance_proto_submatrix, instance_proto_idx_node_submap, node_metadata_dict):
	...
	level_var = z3.Int(f"level_{level}")
	# print("ADDED", level_var)
	...
	for instance_index in idx_node_submap_instance_only.keys():
		...
		instance_index_var = z3.Int(f"i_subA_{instance_index}")
		# print("ADDED", instance_index_var)
		...
		if layer_id in no_consecutive_repeat_layers:
			...
			opt.add(z3.Implies(z3.And(instance_index_var != end(level_var)), proto_parent(level_var, instance_index_var) != proto_parent(level_var, succ(instance_index_var)))) 
		...
		for proto_id in valid_proto_ids:
			...
			proto_index_var = z3.Int(f"i_subA_{proto_index}")
			# print("ADDED", proto_index_var)
			# assign proto parents
			opt.add(z3.Implies(instance_proto_submatrix[proto_index][instance_index], proto_parent(level_var, instance_index_var) == proto_index_var))
...
def add_instance_parent_count_constraints(combined_submatrix, 
	...
	for i_subA2, node_id2 in idx_node_submap2.items():
		i_subA2_var = z3.Int(f"i_subA_{i_subA2}")
		# print("ADDED", i_subA2_var)
		...
		for list_idx, (parent_index1_subA1, parent_id1) in enumerate(list(idx_node_submap1.items())):
			parent_index1_subA1_var = z3.Int(f"i_subA_{parent_index1_subA1}")
			# print("ADDED", parent_index1_subA1_var)
			...
			opt.add(z3.Implies(z3.And(parent_count == 1, parent_condition1), 
											z3.And(instance_parent1(i_subA2_var) == parent_index1_subA1_var, 
														instance_parent2(i_subA2_var) == parent_index1_subA1_var, 
														rank(instance_parent1(i_subA2_var)) == rank(instance_parent2(i_subA2_var)))))
			...
			for parent_index2_subA1, parent_id2 in list(idx_node_submap1.items())[list_idx+1:]: # ensure we're only looking at distinct tuples of parents, otherwise we are UNSAT
				parent_index2_subA1_var = z3.Int(f"i_subA_{parent_index2_subA1}")
				# print("ADDED", parent_index2_subA1_var)
				...
				opt.add(z3.Implies(z3.And(parent_condition1, parent_condition2, parent_count == 2), 
													z3.And(instance_parent1(i_subA2_var) == parent_index1_subA1_var, 
																instance_parent2(i_subA2_var) == parent_index2_subA1_var, 
																rank(instance_parent1(i_subA2_var)) < rank(instance_parent2(i_subA2_var)))))
...
def add_intra_level_linear_chain_for_single_node_level(level, idx_node_submap):
	...
	level_var = z3.Int(f"level_{level}")
	i_subA_var = z3.Int(f"i_subA_{i_subA}")
	# print("ADDED", level_var)
	# print("ADDED", i_subA_var)
...
def define_linearly_adjacent_instance_relations(A_submatrix):
	for i_subA in range(len(A_submatrix)):
		for j_subA in range(len(A_submatrix)):
			if i_subA != j_subA:  # Avoid self-loops
				edge_i_to_j = A_submatrix[i_subA, j_subA]
				i_subA_var = z3.Int(f"i_subA_{i_subA}")
				j_subA_var = z3.Int(f"i_subA_{j_subA}")
				# print("ADDED", i_subA_var)
				# print("ADDED", j_subA_var)
				opt.add(z3.Implies(edge_i_to_j, z3.And(succ(i_subA_var) == j_subA_var, rank(i_subA_var) < rank(j_subA_var)))) # again, these indices are all w.r.t. the SINGLE LEVEL INSTANCE ONLY partition

def add_instance_parent_relationship_constraints(parent_level, child_level, idx_node_submap):
	child_level_var = z3.Int(f"level_{child_level}")
	parent_level_var = z3.Int(f"level_{parent_level}")
	# print("ADDED", child_level_var)
	# print("ADDED", parent_level_var)
	...
	for i_subA, node_id in idx_node_submap.items(): 
		...
		if layer_id in non_overlap_layers: # rules for contiguous, non-overlapping layers
			...
			opt.add(z3.Implies(i_subA_var != end(child_level_var), rank(instance_parent2(i_subA_var)) <= rank(instance_parent1(succ(i_subA_var))))) 
			...
			opt.add(z3.Implies(i_subA_var != end(child_level_var), rank(instance_parent1(i_subA_var)) <= rank(instance_parent1(succ(i_subA_var)))))
		opt.add(z3.Implies(i_subA_var == start(child_level_var), instance_parent1(i_subA_var) == start(parent_level_var))) # the first node must have the prev level's first node as a parent
		opt.add(z3.Implies(i_subA_var == end(child_level_var), instance_parent2(i_subA_var) == end(parent_level_var))) # the final node must have the prev level's last node as a parent
...
for (parent_level, child_level), (A_combined_submatrix, combined_idx_node_submap) in sorted(A_adjacent_instance_submatrices_list.items()):
	...
	# print("DOMAIN EXAMPLE1", idx_node_submap1.keys())
	# print("DOMAIN EXAMPLE2", idx_node_submap2.keys())
	# max_vars = max(len(idx_node_submap1), len(idx_node_submap2))
	# domain_indices = [z3.Int(f'i_subA_{i}') for i in range(max_vars)]
	# c = []
	# for input in domain_indices:
	# 	print("input", input)
	# 	print("max", max_vars)
	# 	c.append(input >= 0)
	# 	c.append(input < max_vars)
	
	# max_levels = 5
	# domain_levels = [z3.Int(f'level_{i}') for i in range(max_levels)]
	# for input in domain_levels:
	# 	print("input", input)
	# 	print("max", max_levels)
	# 	c.append(input >= 0)
	# 	c.append(input < max_levels)
	# opt.add(z3.And(c))
	# funcs = [instance_parent1, instance_parent2, proto_parent, succ, start, end, rank]
	# for f in funcs:
	# 	for input in domain_indices:
	# 		opt.add(f(input) >= 0)
	# 		opt.add(f(input) < max_vars)
