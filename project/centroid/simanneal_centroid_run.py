...
def align_graph_pair(A_G1, A_G2, index_node_mapping):
...
def optimal_alignments_for_graph(A_g, graph_list, index_node_mapping):
  aligned_graph_list = []
  list_alignments = []
...
def initial_centroid_and_alignments(list_of_graphs, index_node_mapping):
  ...
  for A_g in list_of_graphs:
    current_loss, alignments = optimal_alignments_for_graph(A_g, list_of_graphs, index_node_mapping)
