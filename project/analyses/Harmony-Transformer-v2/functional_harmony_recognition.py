...
def translate_predictions(data, pred_cc_values, pred_k_values, pred_r_values, eval_mask_values):
	...
	for seq_idx in range(num_sequences):
		...
		for t_idx, t in enumerate(original_timesteps):
				...
				if val_r == 9 * 14 * 10 * 4:  # Padding
					# Ideally, we shouldn't encounter padding as a roman numeral prediction, since we're only looking
					# at the valid positions after applying eval_mask. however, the model may make inaccurate predictions
					roman_padding = True
				...
				curr_chord = {
					...
					# 'chord_change': val_cc,
					'key': val_k,
					'degree1': inv_degree1_dict[degree1],
					'degree2': inv_degree2_dict[degree2],
					'quality': inv_quality_dict[quality],
					'inversion': inversion
				}

				if not (roman_padding or key_padding or are_chords_equal(curr_chord, prev_chord)): 
					if piece_id not in results:
							results[piece_id] = []
	...
	for piece_id, chords in results.items():
		dir_name = piece_id
		piece_name = os.path.basename(os.path.normpath(dir_name))
		if not os.path.exists(dir_name):
			print("ERROR: DIR", dir_name, "DOES NOT EXIST")
			sys.exit(0)
		out_file = os.path.join(dir_name, f'{piece_name}_functional_harmony.txt')
		with open(out_file, 'w') as f:
			for chord in chords:
				chord_native = {k: convert_to_native(v) for k, v in chord.items()}
				f.write(json.dumps(chord_native) + '\n')
		...
		def predict():
			with tf.Session() as sess:
				...
				for c in process_list:
					filepath = f"/home/ubuntu/project/Harmony-Transformer-v2/preprocessed_data/datasets_{c}_MIREX_Mm/corpus_reshape.pickle"
...
def main():
	...
	for c in process_list:
		...
		dir_path = f'/home/ubuntu/project/Harmony-Transformer-v2/preprocessed_data/datasets_{c}_MIREX_Mm'
