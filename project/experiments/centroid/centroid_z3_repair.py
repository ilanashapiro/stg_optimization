...
def repair_centroid(composer):
	approx_centroid_dir = f"{DIRECTORY}/experiments/centroid/final_centroid/{composer}/soft"

	approx_centroid_path = os.path.join(approx_centroid_dir, "final_centroid.txt")
	approx_centroid = np.loadtxt(approx_centroid_path)
	print(f'Loaded: {approx_centroid_path}')

	approx_centroid_idx_node_mapping_path = os.path.join(approx_centroid_dir, "final_centroid_idx_node_mapping.txt")
	...
	approx_centroid_dir = f"{DIRECTORY}/experiments/centroid/approx_centroid_35s/{composer}"
	...
	final_centroid_dir = f'{DIRECTORY}/experiments/centroid/final_centroid/{composer}/hard'
	...
	final_idx_node_mapping_filename = f'{final_centroid_dir}/final_centroid_idx_node_mapping.txt'

	z3_repair.run(final_centroid_filename, final_idx_node_mapping_filename)

if __name__ == "__main__":
	approx_centroids_dir = f"{DIRECTORY}/experiments/centroid/approx_centroid_35s"
	...
	repair_centroid("bach")

	# with Pool() as pool:
	# 	alignments = pool.map(repair_centroid, [approx_centroid_composers[0]])
	
	# centroid = np.loadtxt("final_centroid_35s/bach/final_centroid.txt")
	# with open("approx_centroid_35s/bach/node_metadata_dict.txt", 'r') as file:
	# 	node_metadata_dict = json.load(file)
	# with open("final_centroid_35s/bach/final_centroid_idx_node_mapping.txt", 'r') as file:
