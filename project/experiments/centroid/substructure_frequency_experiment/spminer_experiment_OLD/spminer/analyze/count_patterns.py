...
def arg_parse():
    ...
    parser.set_defaults(dataset="stg",
                        queries_path=f"results/out-patterns_{COMPOSER}.p",
...
if __name__ == "__main__":
    ...
    if args.dataset != "stg":
      ...
      target = composer_centroids_dict[COMPOSER]
      for node in target.nodes:
        if node.startswith("Pr"):
          target.nodes[node]['label'] = node
        else:
          target.nodes[node]['label'] = node[0]
      for u, v, data in target.edges(data=True):
        source_label = target.nodes[u]['label']
        sink_label = target.nodes[v]['label']
    ...
    query_lens = [len(query) for query in queries] # NUM NODES IN GRAPHS
    # print(queries[0].nodes(data=True))
    # sys.exit(0)
    ...
    if args.baseline == "exact":
        ...
        n_matches = count_graphlets(baseline_queries, targets,
            n_workers=args.n_workers, method=args.count_method,
            node_anchored=args.node_anchored)
    print(f"QUERY LENS: {query_lens}. MATCHES: {n_matches}")
