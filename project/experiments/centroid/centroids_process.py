...
def load_centroids(final_centroids_dir):
	centroids_dict = {}
	for composer in ["bach", "beethoven", "haydn", "mozart"]:
		final_centroid_dir = f"{DIRECTORY}/experiments/centroid/final_centroid_{TIME_PARAM}/{composer}"
		...
		approx_centroid_dir = f"{DIRECTORY}/experiments/centroid/approx_centroid_{TIME_PARAM}/{composer}"
...
def get_composer_graphs_between_duration(selected_composer, min_duration, max_duration):
	...
	return [piece_info[0] for piece_info in centroid_gen_clusters.filter_by_max_min_duration_cluster(composer_graphs, max_duration, min_duration=min_duration)[selected_composer]] # remove duration/STG file size info from the tuple
...
def get_composer_test_graph_filepaths():
	...
	if False:#os.path.exists(test_data_fp):
		...
		for composer, training_pieces_paths in composer_training_pieces_dict.items(): # we JUST include the piece name for comparison
			composer_training_pieces_dict[composer] = [get_piece_name_from_path(file_path) for file_path in training_pieces_paths]
...
if __name__ == "__main__":
	...
	composer_centroids_dict = load_centroids(centroid_path) 
