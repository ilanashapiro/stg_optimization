...
def filter_by_shortest_cluster(composer_graphs):
	# Flatten all values into a single list and sort them
		...
		for key, tuples in composer_graphs.items():
				for tup in tuples:
						...
						if value not in value_to_keys:
								value_to_keys[value] = []
						value_to_keys[value].append((key, tup))
		all_values.sort()
		...
		# Use a sliding window to find all 10-second intervals with the most values
		start_idx = 0
		for end_idx in range(len(all_values)):
				while all_values[end_idx] - all_values[start_idx] > 7:
						start_idx += 1
		...
		for window_start, window_end in best_windows:
				...
				for value in all_values:
						if window_start <= value <= window_end:
								for key, tup in value_to_keys[value]:
...
def find_valid_combination(composers_dict):
		for composer, pieces_info_list in composers_dict.items():
			composers_dict[composer] = [info[0] for info in pieces_info_list] # take out duration/graph size info
		composers_dict['handel'] = ['/home/ubuntu/project/datasets/handel/kunstderfuge/gigue_e_minor_(nc)werths/gigue_e_minor_(nc)werths_augmented_graph_flat.pickle']

		cache_file = "cache.shelve"
		tolerance = 1

		# Open the shelve for caching
	 ...
	 # 0 is most similar
		...
		def is_valid_sequence_for_source_composer(sequence, source_composer_graph_fp, successes_file, failures_file):
			failures = 0
			...
			for i in range(len(sequence) - 1):
				...
				if graph_fp1 == source_composer_graph_fp or graph_fp2 == source_composer_graph_fp:
					continue
				...
				print(source_composer, composer1, d1, rank1)
				print(source_composer, composer2, d2, rank2)

				satisfies_order = d1 <= d2 and rank1 <= rank2 or d1 > d2 and rank1 > rank2

				if not satisfies_order and failures > tolerance:
					return (False, failures)
				elif not satisfies_order:
					failures += 1

			return (True, failures)
		
		# composers sorted alphabetically in each tuple of pieces
		all_combinations = [item for item in list(itertools.product(*(composers_dict[k] for k in sorted(composers_dict.keys()))))]
		...
		for combination in all_combinations:
			print(combination)
			if combination not in saved_successes and combination not in saved_failures:
				entire_combo_valid = True
				total_failures = 0
				for source_piece_fp in combination:
					result_bool, num_failures = is_valid_sequence_for_source_composer(combination, source_piece_fp, successes_file, failures_file)
					if not result_bool:
						entire_combo_valid = False
					else:
						total_failures += 1
				if entire_combo_valid:
...
if __name__ == "__main__":
	...
	if os.path.exists(filtered_composer_graphs_path):
		...
		filtered_composer_graphs = filter_by_shortest_cluster(composer_graphs)
	...
	total_composers = 0
	cluster = filtered_composer_graphs[3]
	assert len(cluster) == 7
	# for composer, graphs in cluster.items():
	#   print(composer, len(graphs))
	# print()
	print(find_valid_combination(cluster))
