...
def get_feature_vector(midi_filepath):
	# Check if the feature vector is already in cache
...
def vectorize_structural_features(num_sections, section_boundaries, max_sections=10):
...
def combined_feature_vector(midi_file):
	...
	note_vector = vectorize_notes(notes_info, max_notes=100)
	harmony_vector = vectorize_harmony(chord_info, num_pitches=MAX_PITCHES, max_chords=MAX_CHORDS)
	structural_vector = vectorize_structural_features(num_sections, section_boundaries, max_sections=10)
...
def run(clusters_filepath):
	...
	cache_filepath = f'{DIRECTORY}/experiments/structural_dist/baseline1/feature_vector_cache.pkl'
...
if __name__ == "__main__":
	clusters_filepath = f"{DIRECTORY}/experiments/structural_dist/struct_dist_data/clusters_totalnobrahmsnohaydn_mindisttol2_totalordertol2.pkl"
